  ...
  def compile_problem(domain_name, problem_name, operators, methods, predicates, state, tasks, axioms, reward, domain_filename = nil)
    ...
    objects.each {|i|
      if i.instance_of?(String)
        problem_str << "#{i} = '#{i}'\n" if i !~ /^-?\d+(?>\.\d+)?$/
      else problem_str << "#{i.join('_').gsub('.','')} = #{evaluate(i)}\n"
      end
    }
    ...
    start_hash.each_with_index {|(k,v),i|
      problem_str << "    '#{k}' => ["
      problem_str << "\n      [" << v.map! {|obj| obj.map! {|o| o.instance_of?(String) ? o =~ /^-?\d+(?>\.\d+)?$/ ? "'#{o.to_f}'" : o : o.join('_').gsub('.','')}.join(', ')}.join("],\n      [") << "]\n    " unless v.empty?
      problem_str << (start_hash.size.pred == i ? ']' : "],\n")
    }
    # Tasks
    group = []
    tasks.each {|t| group << "    ['#{t.first}'#{', ' if t.size > 1}#{t.drop(1).map! {|o| o.instance_of?(String) ? o =~ /^-?\d+(?>\.\d+)?$/ ? "'#{o.to_f}'" : o : o.join('_').gsub('.','')}.join(', ')}]"}
