module UHyper_Compiler
  ...
  def expression_to_hyper(expression, axioms)
    case expression[0]
    when 'and', 'or'
    ...
    when nil then 'false' # Empty list is false
      ...
      if axioms.assoc(expression[0]) then "#{expression[0]}(#{terms})"
      else "@state[#{evaluate(expression[0])}].include?([#{terms}])"
      end
    end
  end
  ...
  def expression_to_s(expression)
    expression.instance_of?(Array) ? "(#{expression.map {|e| expression_to_s(e)}.join(' ')})" : expression
  end
  ...
  def call(expression, namespace = '')
    ...
    # N-ary math
    when '+', '-'
      raise "Expected at least 2 arguments for #{expression_to_s(expression)}" if expression.size < 3
      floats = true
    ...
    when  '*', '/', '%', '^'
      raise "Expected at least 3 arguments for #{expression_to_s(expression)}" if expression.size < 4
      floats = true
      tokens = expression.drop(2).map! {|token|
        token = evaluate(token, namespace, false)
        if token.match?(/^-?\d/) then token = token.to_f
    ...
    when 'abs', 'sin', 'cos', 'tan'
      raise "Expected 2 arguments for #{expression_to_s(expression)}" if expression.size != 3
    ...
    # Comparison
    when '=', '!=', '<', '>', '<=', '>='
      raise "Expected 3 arguments for #{expression_to_s(expression)}" if expression.size != 4
      ltoken = evaluate(expression[2], namespace, false)
    ...
    else raise "Expected function name for #{expression_to_s(expression)}"
    end
  end
  ...
  def compile_domain(domain_name, problem_name, operators, methods, predicates, state, tasks, axioms, rewards, attachments, hypertension_filename = "#{__dir__}/Hypertension_U")
    ...
    # Operators
    define_operators = ''
    operators.each {|name,param,precond_expression,*effects|
